...
function addGoogleBook(bookId) {
  ...
  loadBookById(bookId).then(res => {
    newBook = {
      ...
      listPrice: res.saleInfo,
      reviews: []
    }
    gBooks.push(newBook);
    utilService.saveToStorage(STORAGE_KEY, gBooks);
  });
}
...
function getNextBook(bookId) {
  ...
  if(bookIdx + 1 === gBooks.length){
    return gBooks[gBooks.length-1].id;
  } 
  else return gBooks[bookIdx + 1].id;
}
      ...
      [
        {
          ...
          "publishedDate": 1999,
          "description": "placerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus interdum ad dictum platea vehicula conubia fermentum habitasse congue suspendisseplacerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus interdum ad dictum platea vehicula conubia fermentum habitasse congue suspendisseplacerat nisi sodales suscipit tellus tincidunt mauris elit sit luctus ",
          "pageCount": 713,
          "categories": [
            "Computers",
            "Hack"
